...
function Search(props){
    ...
    function sugItemClickHandler(e){
        console.log('suggestion clicked',e.target.id);
    ...
    let suggestions_list = props.sugList.map((item,i)=>{
        return <div className="sugItem" onClick={sugItemClickHandler} key={i} id={'sugItem-'+item['postId']}>
            {item['title']}
        </div>
    })
    ...
    return <>
        ...
        <div className="search">
            <h1>Search Posts based on their title and content.</h1>
            <h3>Click on search icon, then type your keyword.</h3>
            <div style={{overflow:'visible',position:'relative'}}>
                {/* {search_inp} */}
                <input type="text" id='post-search-input' placeholder="Search . . ." onChange={(e)=>searchTodo(e.target.value)} 
                    onBlur={(e)=>{
                ...
                <div className={'suggestions '+(hide_sug?'hide':'')} tabIndex='-1' style={{position:'absolute'}} onClick={
                    ...
                    onBlur={(e)=>{
                        if(!e.relatedTarget || e.relatedTarget.id.trim()!=='post-search-input'){
                            setHide_sug(true);
                        }
                    }}
                >
